...
* [s] **Aggregation pipeline** — `stats` is now a terminal OQL stage: `stats count by srcIP`, `stats sum(bytes) by hostId`, `stats avg|min|max(field)`, `stats dc(user) by hostId` (distinct count). Grammar in `internal/oql/oql.go` (`parseStats`/`parseFn`); execution in `siem_service.go` (`applyAgg`/`reduce`) over the filtered result window (scan widened to 10k events for aggregations). Response carries `aggregation:{kind,fn,field,by,buckets:[{key,value}]}` alongside the existing envelope; buckets sorted value-desc with key tiebreak so forensic reports render deterministically. Non-numeric values are skipped (not zero-counted) by sum/avg/min/max. Bonus stage: `top N field` (count-by + desc + first N). `as`-aliases, `values`, `list` still open.
* [s] **`timechart` stage** — `timechart span=5m count by severity` bins events into fixed UTC-truncated buckets; per-by-field counts land in `buckets[].series`; default span 1h. Frontend wired: pipe queries route to `/api/v1/siem/oql` (`siemOQL` in bridge.ts); aggregation results render as a bar-chart panel replacing the event table in the SIEM view; filter chips merge into the leading expression so they never trail a terminal stage. Tests: `oql_v2_test.go` (grammar incl. error cases + agg-must-be-terminal), `siem_agg_test.go` (count-by, numeric fns, dc, topN, missing-by bucket, timechart series/ordering). Verified end-to-end against a live server.
...
* [s] **Alert persistence** — shipped as an append-only journal, not BadgerDB: `internal/services/alert_service.go` `AttachJournal(dir)` opens `{dataDir}/alerts.log` (line-delimited JSON, fsync per write) following the proven `cases.log`/`lineage.log` pattern. Every `Raise` and every lifecycle transition (ack/assign/resolve/reopen) appends full alert state; replay-on-startup reduces last-write-wins by ID, preserving first-appearance order so `Recent()` stays chronological. Ring cap (5000) enforced after replay. Wired in `platform/stack.go` (attach on boot, close on shutdown). Tests: `alert_persist_test.go` — three-lifetime restart roundtrip preserving verdict/notes/timestamps + journal-less in-memory fallback; verified end-to-end (resolve → server restart → state intact). Rationale for journal over BadgerDB: alerts are a small bounded set (cap 5000); a hot-store prefix + WAL + cursor pagination can layer on later without changing the API. The remaining items below still stand.
* [s] **Alert search & filter API** — `GET /api/v1/alerts?state=&severity=&ruleId=&hostId=&assignedTo=&from=&to=&q=&limit=`. `AlertService.Query` filters the persisted ring: multi-value `severity` ("high,critical"), case-insensitive `q` against message + ruleName, unix `from`/`to` bounding Triggered, "closed"→"resolved" synonym honoured on both sides of the state match. Results sorted newest-triggered-first (not append order) so backfilled/replayed alerts land where analysts expect. Limit cap 1000. Cursor pagination deferred until the ring outgrows its 5000 cap alongside the BadgerDB move. Bridge: `listAlerts(filter)` back-compat overload. Tests: `alert_query_test.go` covers every filter + ordering + state synonym; verified E2E.
* [s] **Alert metrics endpoint** — `GET /api/v1/alerts/metrics?window=7d` (accepts Go durations or `<N>d` shorthand; bad input → 400). Returns `{windowHours, total, byState, bySeverity, byRule[{ruleId,ruleName,count}], meanTimeToAck_seconds, meanTimeToResolve_seconds}` computed over alerts triggered in-window, straight from the persisted ring. ByRule sorted count-desc with ruleId tiebreak. `fpRate` intentionally omitted — it already lives at `/api/v1/detection/rules/effectiveness`; duplicating it here would create two sources of truth. Surfaced in Detection view: "Alerts (7d)" + "MTTA / MTTR" KPI tiles. Tests cover window exclusion, state/severity/rule breakdowns, and MTTA ≈ trigger→ack delta.
* [s] **Alert export** — `GET /api/v1/alerts/export.csv?from=&to=&state=&severity=&ruleId=&hostId=` streams a CSV attachment with the documented column set (mitre joined with `;`). Reuses `AlertService.Query` so export and API can never disagree. CSV↓ link in the Detection view header follows the active state filter. Verified E2E: resolved alert exports with verdict + RFC3339 lifecycle timestamps.
...
* [s] **Windows Event Log XML parser** — `internal/parsers/winevt.go`. Parses the WEC/`wevtutil qe /f:xml`/Winlogbeat XML shape via `encoding/xml`: System block (`EventID`, `Provider`, `Channel`, `Computer`→HostID, `Level`, `EventRecordID`, `Security UserID`, `TimeCreated SystemTime`→Timestamp) + every `<Data Name=…>` verbatim into Fields — no renaming, so the session classifier's `TargetUserName`/`IpAddress` and the cmdline reconstructor's `CommandLine`/`ParentProcessName` keys line up as-is. Level 1–5 → critical/error/warn/info/debug; 4625/1102/4726 floor at warning. Message synthesised as "EventID <id> <label>" with the classic-security-ID label table (4624 Successful Logon, 4625 Failed Logon, …) so the reconstruction engine's substring heuristics fire. Tolerates `<?xml…>` declaration prefix. Sniff: `<Event` or `<?xml` prefix, checked before the syslog `<PRI>` route. EventType `winevt:<id>`. 5 unit tests + 5-line snapshot corpus (4624/4625/4688 encoded-PowerShell/1102/7045); verified E2E through raw ingest → OQL.
...
* [s] **AWS CloudTrail JSON parser** — `internal/parsers/cloudtrail.go`. Parses one record per line (shippers and the importer unbundle `{Records:[…]}` before ingest): `eventName`→`cloudtrail:<name>` eventType, `userIdentity.userName` (ARN fallback)→`user`, `sourceIPAddress`→`srcIP`, region/eventSource/userAgent/accessKeyId/errorCode into Fields, top-level scalar `requestParameters` flattened under `req.*`. `errorCode` or `"Failure"` responseElements → severity warning, so failed console logins feed triage and the auth correlator. Sniff: JSON containing both `"eventVersion"` and `"eventSource"` — checked before the generic JSON route so plain JSON is untouched. 4 unit tests + 5-record snapshot corpus (ConsoleLogin success/failure, RunInstances, AccessDenied, StopLogging); verified E2E.
...
* [s] **LEEF (Log Event Extended Format) parser** — `internal/parsers/leef.go`. Handles both LEEF 1.0 (tab-delimited payload, 5-field header) and 2.0 (6-field header whose sixth field names the delimiter — literal char or `x09`/`0x09` hex). Maps `src`→`srcIP`, `dst`→`dstIP`, `usrName`→`user`; `sev` (1–10, CEF-shaped scale) → severity; all other attributes verbatim. EventType `leef:<eventId>`. Sniff: `LEEF:` prefix. 4 unit tests (tab delim, custom `^` delim, 1.0 default, truncated-header fallback) + 3-line snapshot corpus (QRadar, PAN-OS, FortiGate); verified E2E.
...
* [s] **Parser test corpus** — `internal/parsers/testdata/{winevt,cloudtrail,leef}/` committed alongside the existing auditd samples; `snapshot_test.go` walks all of them and fails on any plain-fallback line, so production-format drift breaks CI. Explicit field-extraction assertions live in `phase98_test.go` (13 tests: srcIP/user/host per format, severity mappings, sniffer disambiguation vs plain JSON and RFC3164). GCP/Okta/Azure/PAN corpus lands with their parsers.
