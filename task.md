...
- [v] **Setup Wizard** — `frontend-web/src/pages/SetupWizard.svelte` ships a 4-step MVP: admin account → alert channel (none/email/webhook/Slack) → detection pack selection (essential/extended/paranoid) → orientation tutorial. Wired into `App.svelte` at `/setup` (public route). POSTs to `/api/v1/setup/initialize` (already routed) on completion. Steps deferred from the original 6-step claim: TLS cert (operator infra, handled by `cmd/certgen`) and first log source (subsumed by `Onboarding.svelte` once an agent is online). 🌐
