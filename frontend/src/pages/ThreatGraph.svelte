    ...
    series: [
      {
        ...
        roam: true,
        label: { show: true, position: 'right', color: '#a9b1d6', fontSize: 10 },
        force: { repulsion: 1500, edgeLength: 120 },
        lineStyle: { color: '#f7768e', curveness: 0.2, width: 2 },
        itemStyle: { borderColor: '#1a1b26', borderWidth: 2 },
      },
    ],
  };
</script>
...
<PageLayout title="Threat Relationship Graph" subtitle="Visualizing lateral movement patterns and kill-chain progression logic">
  {#snippet toolbar()}
     ...
     <Button variant="danger" size="sm" icon="⚔️">Simulate Counter-Action</Button>
  {/snippet}

  <div class="flex flex-col h-full gap-5">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
      <KPI label="Detected Chains" value="3" trend="stable" trendValue="Active" variant="critical" />
      <KPI label="Mean Progression" value="48%" trend="up" trendValue="Lateral" variant="warning" />
      <KPI label="Observed Tactics" value="12" trend="stable" trendValue="MITRE Sync" />
      <KPI label="Containment Readiness" value="High" trend="stable" trendValue="SOAR Ready" variant="success" />
    </div>
    ...
    <div class="flex-1 bg-surface-1 border border-border-primary rounded-md p-4 relative overflow-hidden shadow-card">
       <div class="absolute inset-0 opacity-[0.03] pointer-events-none"
         style="background-image: linear-gradient(#f7768e 1px, transparent 1px), linear-gradient(90deg, #f7768e 1px, transparent 1px); background-size: 40px 40px;">
       </div>
       ...
       <div class="absolute top-6 left-6 flex flex-col gap-2 z-20">
          <div class="bg-surface-2/90 border border-error/30 p-3 rounded-md shadow-lg flex flex-col gap-1 min-w-[150px]">
             <Badge variant="critical" dot>HIGH ADVERSARY GRAVITY</Badge>
