...
<script lang="ts">
  ...
  import { User, Shield, ExternalLink, Database, Zap } from 'lucide-svelte';

  const enrichedLogs: Record<string, any>[] = [
...
<PageLayout title="Enrichment Core" subtitle="Unified context explorer: Raw telemetry combined with federated intelligence and identity mapping">
  ...
  <div class="flex flex-col h-full gap-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
      <KPI label="Enriched Events" value="1.4M" trend="up" trendValue="+12k/s" variant="accent" />
      <KPI label="Intel Hits" value="142" trend="stable" trendValue="Active Scan" variant="warning" />
      <KPI label="Geo Attribution" value="Global" trend="stable" trendValue="Synced" variant="success" />
            ...
            {#snippet render({ col: column, row })}
              {#if column.key === 'intel'}
               ...
               {:else if column.key === 'geo'}
                  <Badge variant={row.geo === 'LAN' ? 'accent' : 'muted'} size="xs" class="font-bold">{row.geo}</Badge>
