...
<script lang="ts">
  ...
  import { Globe, Activity, Shield, Zap } from 'lucide-svelte';
...
<PageLayout title="Global Geospatial Map" subtitle="Mapping real-time platform presence and cross-border data flows">
  ...
  <div class="flex flex-col h-full gap-5">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
      <KPI title="Active Zones" value="14" trend="Global" />
      <KPI title="Ingress Points" value="412" trend="Nominal" variant="success" />
      <KPI title="Mean Latency" value="112ms" trend="Optimal" variant="success" />
      <KPI title="Encryption Level" value="v3.1" trend="Hardened" variant="accent" />
    </div>

    <div class="flex-1 bg-surface-1 border border-border-primary rounded-md p-4 relative overflow-hidden shadow-premium">
       <!-- Grid Background -->
       <div class="absolute inset-0 opacity-[0.02] pointer-events-none grayscale" style="background-image: linear-gradient(#7aa2f7 1px, transparent 1px), linear-gradient(90deg, #7aa2f7 1px, transparent 1px); background-size: 60px 60px;"></div>
       ...
       <div class="absolute bottom-6 right-6 flex flex-col gap-2 z-20">
          <div class="bg-surface-2/80 backdrop-blur-md border border-border-secondary p-4 rounded-md shadow-2xl flex flex-col gap-2 min-w-[200px]">
             ...
             <div class="flex justify-between items-center text-[11px] text-text-secondary">
                ...
                <span class="font-bold">Moderate</span>
             </div>
          </div>
       </div>
    </div>
