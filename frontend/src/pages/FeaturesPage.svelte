...
<script lang="ts">
  ...
  import { Zap, Activity, ShieldCheck, Database, Globe, RefreshCcw, Lock, Star } from 'lucide-svelte';
...
<PageLayout title="Platform Features" subtitle="Capability orchestration: Managing platform-wide feature toggles, experimental modules and sovereign-grade entitlements">
  ...
  <div class="flex flex-col h-full gap-6">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <KPI title="Active Flags" value="12" trend="Nominal" />
      <KPI title="Release Channel" value="STABLE" trend="Hardened" variant="success" />
      <KPI title="Entitlement Tier" value="SOVEREIGN" trend="Verified" variant="success" />
      <KPI title="System Agility" value="High" trend="Optimized" variant="success" />
    </div>
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div class="flex-1 overflow-auto">
            <DataTable data={features} columns={[
              ...
              {#snippet cell({ column, row })}
                {#if column.key === 'state'}
                   <Badge variant={row.state === 'enabled' ? 'success' : row.state === 'experimental' ? 'accent' : 'secondary'}>{row.state.toUpperCase()}</Badge>
      ...
      <div class="flex flex-col gap-6">
         <div class="bg-surface-1 border border-border-primary rounded-md p-6 flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden group border-dashed shadow-sm">
            <Star size={48} class="text-accent opacity-40 animate-pulse" />
