<!--
  ...
  Arming War Mode also activates shell-level Crisis Mode.
-->
<script lang="ts">
  ...
  function armWarMode() {
    if (authCode === 'SIGMA') {
      armed = true;
      crisisStore.arm('WAR MODE: Manual SIGMA-code activation');
  ...
  function standDown() {
    armed = false;
    authCode = '';
    crisisStore.standDown();
    appStore.notify('War Mode stood down. Systems resuming normal state.', 'warning');
  }
</script>
    ...
    <div class="flex-1 min-h-0 flex flex-col items-center justify-center p-12 bg-surface-1 border-2 {armed ? 'border-error' : 'border-border-primary'} rounded-md relative overflow-hidden">
        ...
        <div class="max-w-md w-full flex flex-col items-center gap-8 text-center relative z-10">
          ...
          <div class="space-y-2">
            ...
            <p class="text-[11px] text-text-muted leading-relaxed">
              Initiating War Mode will immediately terminate all active sessions, lock all vaults, and rotate platform root keys.
              This action cannot be reversed without hardware intervention.
            </p>
          </div>
      ...
      {:else}
        <div class="max-w-md w-full flex flex-col items-center gap-8 text-center relative z-10">
          ...
          <Button variant="secondary" size="sm" onclick={standDown}>STAND DOWN (FORCE RE-AUTH)</Button>
        </div>
      {/if}
    </div>
