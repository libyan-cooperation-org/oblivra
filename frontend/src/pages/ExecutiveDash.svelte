...
-->
<script lang="ts">
  import { Chart, PageLayout, KPI, Badge } from '@components/ui';
    ...
    series: [
      {
        ...
        itemStyle: { borderRadius: 4, borderColor: '#1a1b26', borderWidth: 2 },
        label: { show: false, position: 'center' },
        emphasis: { label: { show: true, fontSize: 14, fontWeight: 'bold', color: '#a9b1d6' } },
...
<PageLayout title="Executive Overview" subtitle="High-level security posture and fleet health metrics">
  <div class="flex flex-col h-full gap-5">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
      ...
      <KPI label="Fleet Risk Score" value="12.4" variant="accent" />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 flex-1 min-h-0">
      <div class="bg-surface-1 border border-border-primary rounded-md p-6 flex flex-col shadow-card">
    ...
    <div class="p-4 bg-accent/5 border border-accent/20 rounded-md shrink-0">
       <div class="flex justify-between items-center">
          <div class="flex items-center gap-3">
             <div class="w-2 h-2 rounded-full bg-accent animate-ping shrink-0"></div>
